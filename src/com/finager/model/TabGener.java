...
public class TabGener {
	...
	public ProVal[] Ranking(){
		r = new Ranking (d2,"Household final consumption expenditure ");
		return this.r.getRank();
	}
	/**
	 * Find all big category value by given provinces
	 * @param pc-the given province 
	 * @return-all big category value of the given province
	 */
	
	public Vector<String> FindProv(String pc){
		//create a variable to store the result
		Vector<String> result = new Vector<String>();

		//initialize Ranking class and sort different categories
		for (Integer inx : f.indexes()){
			String catg = f.CatgNames().get(inx);
			//sort the big categories
			r = new Ranking (d2,catg);
			//search for given province
			String val = String.format("%.2f", this.r.find(pc));
			//combine 
			String temp = catg + ": " + val;
			result.add(temp);
		}
		return result;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(run.Name(0));
	
		System.out.println(Arrays.toString(run.Ranking()));
		System.out.println(run.FindProv("Canada"));
